...
def beam_with_VI_vibrations(alpha_coef_lst_L, alpha_coef_lst_R, beta_coef_lst_L, beta_coef_lst_R, gamma_, just_VI=False):
    ...
    # while True:
        ...
        if (len(energy_data_global) > 0) and (energy_data_global[-1] <= energy_data_global[0] / np.exp(1)) and first_time_tau:
            ...
            print(f'Number VI = {number_VI}')
            print('----------------------------------')
            graph(np.concatenate((u_next_L, u_next_R[1:]), axis=0), np.concatenate((v_next_L, v_next_R[1:]), axis=0), energy_over_modes_lst[:num_modes_plot_energy], case_plot=False, case_time_delay=False, case_time_delay_2=True)
...
print(finding_max_number_mode(gamma_, 1.0, num_modes))
print('For left part')
print(finding_max_number_mode(gamma_, a_bar, num_modes))
print('For right part')
print(finding_max_number_mode(gamma_, (1 - a_bar), num_modes))
# ---------------------------------
...
omega_m_damping = lambda m, gamma_cur, string_length=1: (np.pi * m / string_length) * np.sqrt(1 - ((np.pi * m / string_length) ** 2 * gamma_cur ** 2) / 4)
