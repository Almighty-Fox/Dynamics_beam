...
F_MIN = 0.6e7  # Ньютон
F_MAX = 1.0e7  # Ньютон
F_STEP = 4.0e5  # шаг по сетке нагрузок
T_RAMP = 1.0  # с — время линейного нарастания
T_FINAL = 3.0  # с — суммарное время интегрирования
VEL_TOL = 1e-3  # м/с — «достаточно успокоилось»
CSV_OUT = Path("final_positions.csv")
...
# ------------------------------------------------------------
def make_eom(F1_amp: float, F2_amp: float):
    """Return eom(t, y) with *embedded* amplitudes."""

    def force(t, amp):
        # Linear ramp 0 → amp (downwards) over T_RAMP, then zero
        if t < T_RAMP:
            return -amp * (t / T_RAMP)
        else:
            return 0.0
...
# ------------------------------------------------------------
F_values = np.arange(F_MIN, F_MAX + F_STEP / 2, F_STEP)
results = []  # list of (F1, F2, y1_final, y2_final)

initial_state = np.array([1.0, 1.0, 0.0, 0.0])

for F1_amp in F_values:
    for F2_amp in F_values:
        eom = make_eom(F1_amp, F2_amp)
        ...
        if np.hypot(y_last[2], y_last[3]) > VEL_TOL:
            print(f"⚠️  (F1={F1_amp: .1e}, F2={F2_amp: .1e}) "
                  f"к T_FINAL скорость ещё {np.hypot(y_last[2], y_last[3]):.2e} м/с")

        results.append((F1_amp, F2_amp, y1_fin, y2_fin))

        print(f"• F1={F1_amp: .1e}, F2={F2_amp: .1e}  →  "
...
with CSV_OUT.open("w", newline="") as f:
    ...
    writer.writerow(["F1_max [N]", "F2_max [N]", "y1_final [m]", "y2_final [m]"])
...
file_path = 'final_positions.csv'  # Убедитесь, что путь корректен
...
plt.scatter(df['F1_max [N]'], df['F2_max [N]'], c=df['color'])
plt.xlabel('F1_max [N]')
plt.ylabel('F2_max [N]')
