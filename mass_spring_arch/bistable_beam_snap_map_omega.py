...
# -*- coding: utf-8 -*-
"""
Карта минимальной амплитуды гармонической точечной силы F0* как функции
частоты внешнего воздействия Ω (в долях опорной линейной частоты ω_ref)
для бистабильной фикс-фикс балки. Точка приложения силы близка к центру
(не совсем по центру, чтобы ввести небольшую асимметрию).

Что делает скрипт:
1) Собирает модальную МКЭ-модель балки (энерг. нормировка по ∫(φ')² dx = 1).
2) Находит линейный спектр, определяет ω_ref (первая положительная ω).
3) Для заданной фиксированной точки x_s ≈ L/2 ищет для каждого Ω из сетки
   порог F0*, при котором q1(t) за N_periods периодов достигает
   q1 ≤ −θ·q_eq (перещёлкивание).
4) Сохраняет результаты в NPZ/CSV и строит графики:
   - F0*(Ω) vs Ω (безразмерно) и F0*(Ω) vs f=ω/(2π), Гц.

Зависимости: numpy, scipy, matplotlib, tqdm (необязательно)
"""
...
# Критическая нагрузка Эйлера для фикс-фикс: Pcr = 4π² E I / L²
...
# === Скан по Ω (безразмерно, ω_drive = Ω * ω_ref) ===
Omega_min = 0.2
Omega_max = 100.0
n_Omega   = 100
Omega_log_scale = False   # логарифмически распределённая сетка

# Точка приложения силы: почти центр (вводим небольшую асимметрию)
center_offset_frac = 0.1     # 2% от L вправо от центра; можно поставить отрицат. для влево
x_s = 0.5*L + center_offset_frac*L

# Временные настройки интегрирования
n_periods = 120          # число периодов воздействия в одном прогоне
rtol, atol = 1e-6, 1e-9
max_step_per_period = 1/200.0  # не более 1/200 периода на шаг интегратора
...
# Сборка модели
# =============================================================
xgrid = np.linspace(0.0, L, 1201)
Phi, dPhi, ddPhi = build_modes(L, N_modes, xgrid)
...
for i in range(N_modes):
    for j in range(i, N_modes):
        val = rhoS * np.trapz(Phi[i]*Phi[j], xgrid)
        M[i,j] = val; M[j,i] = val

# Линейная жёсткость: K_i = E I ∫ (φ''_i)^2 dx − P
K_diag = np.array([E*I*np.trapz(ddPhi[i]**2, xgrid) - P for i in range(N_modes)])
K = np.diag(K_diag)
...
phi_xs = np.array([np.interp(x_s, xgrid, Phi[i]) for i in range(N_modes)])

# =============================================================
# RHS и событие «snap»
# =============================================================
def make_rhs(excitation_omega: float, F0: float):
    def rhs(t, y):
        q  = y[:N_modes]
        qd = y[N_modes:]
...
# =============================================================
def run_single(F0: float, excitation_omega: float, t_final: float, max_step: float) -> bool:
    rhs = make_rhs(excitation_omega, F0)
...
def find_threshold_F0_for_Omega(Omega: float, F0_hint: float | None = None) -> float:
    # ω_drive для данного Ω
    exc_omega = max(Omega * max(omega_ref, 1e-12), 1e-12)
    T_drive = 2.0*np.pi / exc_omega
    max_step = max_step_per_period * T_drive
    t_final = n_periods * T_drive

    lo = F0_min_start if F0_hint is None else max(1e-12, 0.5*F0_hint)
    hi = lo
    snapped = run_single(hi, exc_omega, t_final, max_step)

    # экспоненциальный рост до срабатывания или потолка
    grow = 1.8
    ...
    # двоичный поиск порога
    while (hi - lo) > bisection_tol * hi:
        mid = 0.5*(lo + hi)
        if run_single(mid, exc_omega, t_final, max_step):
            hi = mid
        else:
            lo = mid
    return hi
...
# =============================================================
if Omega_log_scale:
    Omega_grid = np.geomspace(max(Omega_min, 1e-3), max(Omega_max, Omega_min*1.01), n_Omega)
else:
...
print("Поиск порога F0 по частоте Ω при почти-центральном форсинге...")
print(f"P/Pcr={P/Pcr:.3f}; x_s/L={x_s/L:.4f}; ω_ref={omega_ref:.4g} rad/s")

for Om in tqdm(Omega_grid, desc="Ω scan"):
    F_thr = find_threshold_F0_for_Omega(Om, F0_hint=F_hint)
    F0_min_list.append(F_thr)
    exc_omega_list.append(max(Om * max(omega_ref, 1e-12), 1e-12))
    if not np.isnan(F_thr):
        F_hint = F_thr

F0_min = np.array(F0_min_list)
exc_omega_arr = np.array(exc_omega_list)
exc_freq_hz = exc_omega_arr / (2*np.pi)

# # Сохранение результатов
# np.savez_compressed(f"{save_prefix}_results.npz",
#                     Omega_grid=Omega_grid, F0_min=F0_min,
#                     exc_omega=exc_omega_arr, exc_freq_hz=exc_freq_hz,
#                     x_s=x_s, L=L, P=P,
#                     n_periods=n_periods, theta=theta_threshold,
...
#     w = csv.writer(f)
#     w.writerow(["Omega", "omega_drive_rad_s", "f_drive_Hz", "F0_min_N", "x_s_over_L"])
#     for Om, wdrv, fdrv, F in zip(Omega_grid, exc_omega_arr, exc_freq_hz, F0_min):
#         w.writerow([f"{Om:.6g}", f"{wdrv:.6g}", f"{fdrv:.6g}", f"{F:.6g}", f"{x_s/L:.6g}"])

# =============================================================
# Визуализации
# =============================================================
if make_plots:
    # 1) F0_min(Ω) — безразмерная частота
    plt.figure(figsize=(8.2, 4.6))
    plt.plot(Omega_grid, F0_min, marker="o", lw=1.5)
    plt.xscale('log' if Omega_log_scale else 'linear')
    plt.xlabel(r"безразмерная частота, $\Omega = \omega_\mathrm{drive}/\omega_\mathrm{ref}$")
    plt.ylabel(r"минимальная амплитуда силы $F_0^*$, Н")
    plt.title(r"Порог перещёлкивания $F_0^*$ vs $\Omega$ (почти-центр: $x_s/L=%.3f$)" % (x_s/L))
    plt.grid(True, ls='--', alpha=0.35, which='both')
    plt.tight_layout()
    plt.savefig(f"{save_prefix}_F_vs_Omega.png", dpi=180)
    # plt.show()

    # 2) F0_min(f) — абсолютная частота в Гц
    plt.figure(figsize=(8.2, 4.6))
    plt.plot(exc_freq_hz, F0_min, marker="s", lw=1.5)
    plt.xscale('log' if Omega_log_scale else 'linear')
    plt.xlabel(r"частота возбуждения $f = \omega/2\pi$, Гц")
    plt.ylabel(r"минимальная амплитуда силы $F_0^*$, Н")
    plt.title(r"Порог перещёлкивания $F_0^*$ vs $f$ (почти-центр: $x_s/L=%.3f$)" % (x_s/L))
    plt.grid(True, ls='--', alpha=0.35, which='both')
    plt.tight_layout()
    plt.savefig(f"{save_prefix}_F_vs_freqHz.png", dpi=180)
    plt.show()
