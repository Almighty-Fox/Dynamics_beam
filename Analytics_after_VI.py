...
w_vnesh = 3 * w_sobst
# www = (14.1372 / l)**2 * np.power((E * Jx / ro / F), (1/2))
# print(1 / (www / 2 / np.pi))
...
axs[1].plot(list_time, y_barrier, 'r', linewidth=1)
...
for t in np.arange(2*step, time_end, step):
    ...
    try:
        ...
        if len(list_time) % step_plot == 0:
            axs[1].plot(list_time, y_end, 'k', linewidth=1)
            axs[1].plot(list_time, y_barrier, 'g', linewidth=1)
            axs[1].grid()
            axs[1].set_title('Black - Beam end coordinate,\nGreen - Point opposite the barrier.', fontsize=10)
            axs[1].set(xlabel='time, sec', ylabel='displacement, m')
            # axs[1].set_ylim(-4e-4, 4e-4)
